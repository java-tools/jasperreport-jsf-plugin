...
public final class Configuration {
	...
	private static final String INSTANCE_KEY = Configuration.class.getPackage().getName();
	...
	public static Configuration getInstance(FacesContext context) throws ConfigurationException {
		if (context == null) {
	...
	private Configuration(FacesContext context) throws ConfigurationException { 
		InputStream is = context.getExternalContext().getResourceAsStream(WEB_XML);
		Document webXml;
		try {
	...
	private void loadMappings(Document webXml) throws ConfigurationException {
		List<Element> servletList = XmlHelper.getChildElements(
				webXml.getDocumentElement(), TAG_SERVLET);
		for(Element servlet : servletList) {
			String servletName = XmlHelper.getChildText(servlet, TAG_SERVLET_NAME);
			String servletClass = XmlHelper.getChildText(servlet, TAG_SERVLET_CLASS);
			if(!FACES_SERVLET_CLASS.equals(servletClass)) continue;
			
			List<Element> servletMappingList = XmlHelper.getChildElements(
					webXml.getDocumentElement(), TAG_SERVLET_MAPPING);
			for(Element servletMapping : servletMappingList) {
				String mappingName = XmlHelper.getChildText(servletMapping, TAG_SERVLET_NAME);
